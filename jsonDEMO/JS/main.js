...
for(let i = 0; i < data.results.forecast.length; i++){
showData = `Today is ${data.results.forecast[i].day}. The high is ${data.results.forecast[i].high}. The low is ${data.results.forecast[i].low}.  It will be ${data.results.forecast[i].text}.`;
console.log(showData);
document.getElementById("print").innerHTML += "<table>" + showData + "</table>"; 
...
showData = ` ${obj.employees[i].firstName} ${obj.employees[i].lastName}`;
let showJob =` ${myJSON.people[i].job}`;
document.getElementById("people").innerHTML += "<p>Full Name:" + showData + " <br/> Job:"+showJob+"</p>"; 

console.log("fullname:" + showData + " job:" + showJob);
//console.table(obj.employees[i]);
